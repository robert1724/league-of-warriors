...
public class Game {
    ...
    public static void main(String[] args) {
        ...
        int grid_length = rand.nextInt(8) + 3; //intre 3 si 10
        int grid_width = rand.nextInt(8) + 3;
        Game.grid = Grid.getInstance(grid_length, grid_width);
        ArrayList<ArrayList<Cell>> game_map = Grid.map_generator(grid_length, grid_width);
        for (int i = 0; i < grid_length; i++) {
            for (int j = 0; j < grid_width; j++) {
                if(game_map.get(i).get(j).getType() == CellEntityType.PLAYER)
                    grid.currentCell = game_map.get(i).get(j);
            }
        }
        ...
        while (ok == 1) {
            ...
            try {
                ...
                if (move.equals("w"))
                    grid.goNorth(game_map);
                else if (move.equals("a"))
                    grid.goWest(game_map);
                else if (move.equals("d"))
                    grid.goEast(game_map);
                else if (move.equals("s"))
                    grid.goSouth(game_map);
